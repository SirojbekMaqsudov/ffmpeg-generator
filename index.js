...
(async () => {
  ...
  function generate(resolution, audioIndex) {
    ...
    const cmd = `ffmpeg -i "${inputPath}" -filter_complex "${filterComplex}" ${outputCmds.join(' ')} -var_stream_map "${streamMap}" -master_pl_name "master.m3u8" -f hls -hls_time 4 -hls_playlist_type vod -hls_segment_filename "${outputPath}/%v/chunk_%03d.ts" "${outputPath}/%v/playlist.m3u8"`;
